...
emptyColumns StarField{stars=stars, width=width } = filter (\x -> S.null $ S.filter (\(x',y') -> x' == x) stars) [0..width-1]

emptyRows :: StarField -> [Int]
emptyRows StarField{stars=stars, height=height} = filter (\y -> S.null $ S.filter (\(x',y') -> y' == y) stars) [0..height-1]
